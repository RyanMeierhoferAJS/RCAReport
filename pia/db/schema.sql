...
CREATE TRIGGER tasks_search_update
    ...
    tsvector_update_trigger(search_vector, 'pg_catalog.english', title, description, project, owner);
...
CREATE TRIGGER decisions_search_update
    ...
    FOR EACH ROW EXECUTE FUNCTION
    tsvector_update_trigger(search_vector, 'pg_catalog.english', title, description, reason, project);
...
CREATE TRIGGER career_search_update
    BEFORE INSERT OR UPDATE ON career_events
    FOR EACH ROW EXECUTE FUNCTION
    tsvector_update_trigger(search_vector, 'pg_catalog.english', title, description, project);
...
-- Notes needs a custom trigger because tags/entities are arrays
CREATE OR REPLACE FUNCTION notes_search_update_fn() RETURNS trigger AS $$
BEGIN
